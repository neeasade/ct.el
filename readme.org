...
Supported color spaces: ~lab~ ~lch~ ~hsl~ ~hsluv~. Generally, scalar values fall within range 0 to 100 (consistent interface across spaces for the convienence of using this libary). Execptions are: Hue (0 to 360), and the AB values in ~lab~, which are -100 to 100.
